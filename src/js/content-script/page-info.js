...
const isImagePage =
  document.images.length == 1 && document.images[0].src == location.href;
...
function getImages() {
  ...
  if (
    ...
    const images = document.querySelectorAll(
      ".permalink-tweet .AdaptiveMedia img"
    );
    return [...images]
      .slice(0, 6)
      .map((img) => img.src)
  ...
  if (
    location.hostname === "500px.com" &&
    /^\/photo\/\d+/.test(location.pathname)
  ) {
