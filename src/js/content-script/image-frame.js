export default class {
  ...
  initHotZones() {
    ...
    if (location.hostname === "petapixel.com") {
      this.hotZones = [
        ...document.querySelectorAll(".image-wrap > img"),
      ].map((img) => HotZone.fromImage(img));
    }
  }
}
...
function getImageSrc(el) {
  ...
  if (location.hostname === "www.flickr.com") {
    if (el.nodeName === "A" && el.classList.contains("overlay")) {
      const m = el.parentNode.parentNode.parentNode.style.backgroundImage.match(
        /^url\("(\/\/.+)"\)$/
      );
      if (m) {
    ...
    if (
      el.nodeName === "DIV" &&
      el.classList.contains("photo-notes-scrappy-view")
    ) {
      return getImageSrc(
        el.previousElementSibling.querySelector("img.main-photo")
      );
    }
  }
  ...
  if (location.hostname === "500px.com") {
    ...
    if (
      el.nodeName === "DIV" &&
      el.classList.contains("nsfw_placeholder_content")
    ) {
