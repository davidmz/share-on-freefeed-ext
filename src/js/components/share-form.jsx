...
export default inject(
  ...
  observer(
    class extends Component {
      ...
      @action changeCommentText = (event) =>
      ...
      constructor(props) {
        ...
        if (location.search.charAt(0) === "?") {
          const { title, url, selection, images } = qs.parse(
          ...
          if (selection) {
            if (ui.settings.selectionPlacement === "post") {
              const quote = /[\u0400-\u04ff]/.test(selection)
                ? `\u00AB${selection}\u00BB`
                : `\u201C${selection}\u201D`;
              postText = `${quote}\n\n${postText}`;
            } else {
      ...
      submit = (event) => {
        ...
        ui.setBlocking();
        api
          .bookmarklet(postText, ui.images.peek(), commentText, feeds)
        ...
        return (
          <div>
            ...
            <form onSubmit={this.submit} className={css.bmkForm}>
              <p>
                <span className="link" onClick={this.showSelector}>
                  To
                </span>
              ...
              <ImageList
                srcList={this.props.ui.images.slice()}
                remover={this.delImage}
              />
              {this.commentOpened ? (
                <p>
                ...
                <p>
                  <span className="link" onClick={this.openComment}>
...
function targetPreview(tgt) {
  return (
    <span className={css.targetPreview} title={"@" + tgt.userName}>
      <img src={tgt.avatar} />
      {tgt.screenName}
    </span>
  );
