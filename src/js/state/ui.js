...
class UIStatus {
  ...
  @action setLoading(st) {
    this.isLoading = st;
  }

  @action setBlocking(b = true) {
    this.isBlocking = b;
  }

  @action setPostAddress(addr) {
    this.postAddress = addr;
  }
  ...
  @action setScrolling(v) {
    this.withScrolling = v;
  }
  ...
  isSelected(feedName) {
    return this.selectedFeeds.indexOf(feedName) >= 0;
  }
