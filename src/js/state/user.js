...
class State {
    ...
    constructor() {
        try {
            ...
            setTimeout(() => {
                api.whoami()
                    .then(resp => {
                        if (this.isAuthorized) { // user may log out during request
    ...
    saveFrontPrefs() {
        if (prefsSaveInProgress) {
            ...
            prefsToSave = false;
            api.saveFrontPrefs(frontPrefsKey, {favorites: toJS(this.favorites)})
                .finally(() => {
    ...
    signIn(userName, password, remember) {
        ui.setBlocking();
        return api.startSession(userName, password)
            .then(resp => {
    ...
    @action rcvWhoamiResponse({users, subscribers}) {
        ...
        this.targets = subscribers
            ...
            .filter(g => g.isRestricted === '0' || (g.administrators || []).indexOf(this.id) >= 0)
